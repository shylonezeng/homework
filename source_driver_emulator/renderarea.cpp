...
{
    rads=15*global_scale;
    scale=30*global_scale;

    current_st=0;
    flag_drawtext=0;
    memset(RenderArea::if_add,0,sizeof(RenderArea::if_add));
...
void set_brushstyle(QPainter &painter,int color_flag=0)
{
    ...
    if(color_flag)
        color=Qt::red;
    QBrush brush(color,bstyle);

    painter.setPen(pen);
    painter.setBrush(brush);
}
int ignore_flag=true;
void RenderArea::once_draw(QPainter& painter,int draw_to)
{
    ...
    for (int i=0;i<draw_to;++i)
    {
        ...
        if((!ignore_flag)&& RenderArea::if_add[i])
        {
            set_brushstyle(painter,1);
            this->drawround_and_move(painter,&x,&y);
            set_brushstyle(painter,0);
        }
        else
            this->drawround_and_move(painter,&x,&y);
